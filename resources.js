// Arquivo de geração de icons e splashScreen
...
function copyImages(
  ...
  for (var icon of images) {
    let source = path.join(__dirname, 'resources-mobile', 'android', isIcon ? 'icon' : 'splash', icon.source);
    let target = path.join(__dirname, 'android', 'app', 'src', 'main', 'res', icon.target);
...
const SOURCE_ANDROID_ICON = 'resources-mobile/android/icon/';
const SOURCE_ANDROID_SPLASH = 'resources-mobile/android/splash/';
...
copyImages(SOURCE_ANDROID_ICON, TARGET_ANDROID_ICON, ANDROID_ICONS, true);
copyImages(SOURCE_ANDROID_SPLASH, TARGET_ANDROID_SPLASH, ANDROID_SPLASHES, false);
